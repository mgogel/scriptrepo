...
# * Login to iCloud account (in System Preferences) (we only enable Find My Mac here)
...
# * Remap caps lock to control on all keyboards
# * Add the date to the clock
# * Remove the User menu item (actually this script should be able to do this but I'm tired and it's not working on the first try)
...
# See: http://ericboehs.com/post/sharing-a-team-heroku-account
...
echo "FIXME: Don't think user-land defaults commands are working :/"
...
curl -L https://github.com/rs/SafariTabSwitching/releases/download/1.2.4/Safari.Tab.Switching-1.2.4.pkg > ~/Downloads/Safari.Tab.Switching.pkg 2>/dev/null
